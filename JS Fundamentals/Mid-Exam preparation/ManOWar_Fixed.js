    ...
    while(line !== "Retire"){
        ...
        switch(tokens[0]){
            case "Fire": 
                ...
                if (i < 0 || i > warShip.length - 1) { //проверяваме дължината на иднекса и да не е отрицателен инпута, за да не се счупи;
                    line = input[index];
                    index++;
                    continue;
                }
            ...
            case "Defend": 
                ...
                if (i < 0 || i > statusPirateShip.length - 1 || endIndex < 0 || endIndex > statusPirateShip.length - 1){ // може да гръмне, ако старт индекса е по-голям от крайния
                    line = input[index];
                    index++
                    continue;
                }
