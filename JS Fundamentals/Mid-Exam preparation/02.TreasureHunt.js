    ...
    switch (operation) {
      ...
      case "Drop":
        ...
        if (i < 0 || i > state.length) {
          command = data[index]; // при невалиден индекс спираме цикъла и връшаме изпълнението 
          //от начало с continue, но все пак трябва да инкрементираме индексът;
          index++;
          continue;
        }
        let element = state.splice(i, 1);
        state.push(element[0]); //задаваме му да изтрие елемент от индекс
        break;
